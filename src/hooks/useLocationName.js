...
const useLocationName = () => {
  const cb = useCallback(async ({ lat, lng }) => {
    ...
    try {
      ...
      const location = data.results.find((r) => r.types[0] === "locality");
      const city =
        location.address_components.find((c) => c.types.includes("locality"))
          ?.long_name || "";
      const state =
      ...
      const country =
        location.address_components.find((c) => c.types.includes("country"))
          ?.long_name || "";
